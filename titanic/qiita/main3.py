...
# CSVを読み込む
train= pd.read_csv(r"D:\program\programming\study\kaggle_comp\titanic\better_model\data\train.csv")
test= pd.read_csv(r"D:\program\programming\study\kaggle_comp\titanic\better_model\data\test.csv")
...
known_age = age_dummies[age_dummies.Age.notnull()].to_numpy()
null_age = age_dummies[age_dummies.Age.isnull()].to_numpy()
...
# trainデータを変数と正解に分離
X = train_set.to_numpy()[:, 1:] # Pclass以降の変数
y = train_set.to_numpy()[:, 0].astype(int) # 正解データ
